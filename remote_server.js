...
app.post('/api/ito/register', async (req, res) => {
  try {
    const { userId, fullName, email, mobile, schoolName, city } = req.body;
    if (!fullName || !email || !mobile || !schoolName) {
      return res.status(400).json({ error: 'All required fields must be filled.' });
    }
    ...
    const result = await db.query(
      `INSERT INTO ito_registrations (user_id, full_name, email, mobile, school_name, city)
       VALUES ($1, $2, $3, $4, $5, $6) RETURNING id, registered_at`,
      [userId || null, fullName, email, mobile, schoolName, city || '']
    );
    res.json({ success: true, registrationId: result.rows[0].id, registeredAt: result.rows[0].registered_at });
  } catch (err) {
...
app.get('/api/ito/registrations', requireAdmin, async (req, res) => {
  try {
    await ensureITORegistrationsTable();
    const result = await db.query(
      `SELECT id, user_id, full_name, email, mobile, school_name, city, registered_at
...
app.post('/api/ito/register', async (req, res) => {
  try {
    ...
    if (!fullName || !email || !mobile || !subjectOrRole || !schoolName || !city) {
      return res.status(400).json({ error: 'All required fields must be filled.' });
    }
    
    // Create table if not exists
    await db.query(`
      CREATE TABLE IF NOT EXISTS gforce.ito_registrations (
        id SERIAL PRIMARY KEY,
        user_id TEXT,
        full_name TEXT,
        email TEXT,
        mobile TEXT,
        subject_or_role TEXT,
        school_name TEXT,
        city TEXT,
        registered_at TIMESTAMPTZ DEFAULT NOW()
      );
    ...
    const emailDup = await db.query(`SELECT id FROM gforce.ito_registrations WHERE email = $1`, [email]);
    if (emailDup.rows.length > 0) {
      return res.status(409).json({ error: 'already_registered', message: 'This email is already registered for ITO.' });
    }

    const result = await db.query(
      `INSERT INTO gforce.ito_registrations (user_id, full_name, email, mobile, subject_or_role, school_name, city)
       VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id, registered_at`,
  ...
  try {
    const result = await db.query(
      `SELECT id, user_id, full_name, email, mobile, subject_or_role, school_name, city, registered_at
       FROM gforce.ito_registrations ORDER BY registered_at DESC`
    );
    res.json({ total: result.rows.length, registrations: result.rows });
  } catch (err) {
    res.status(500).json({ error: err.message });
