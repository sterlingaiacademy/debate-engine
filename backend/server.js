  ...
  try {
    ...
    res.json({
      ...
      user: { 
        ...
        class_section: user.class_section || null
      },
      token
    });
  } catch (err) {
...
app.get('/api/admin/users', requireAdmin, async (req, res) => {
  try {
    ...
    const usersRes = await db.query(
      `SELECT users.name, users."studentId", users.email, COALESCE(NULLIF(users.phone, ''), users.parent_phone) as phone, COALESCE(NULLIF(users.grade, ''), users."classLevel") as grade,
...
// GET /api/teacher2030/certificate-status/:email
app.get('/api/teacher2030/certificate-status/:email', (req, res) => {
  try {
    const email = req.params.email.toLowerCase().trim();
    const certPath = path.join(__dirname, 'teacher2030_certificates.json');
    if (!fs.existsSync(certPath)) {
      return res.status(500).json({ error: 'Certificate database not found.' });
    }
    const certData = JSON.parse(fs.readFileSync(certPath, 'utf8'));
...
// GET /api/teacher/submissions/:teacherId — all submissions for this teacher
