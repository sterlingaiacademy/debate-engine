...
router.post('/analyze', upload.single('audio'), async (req, res) => {
  try {
    ...
    // Calculate active speaking time for precise WPM
    let activeDurationSeconds = 0;
    if (words.length > 0) {
      const firstWordStart = words[0].start;
      const lastWordEnd = words[words.length - 1].end;
      activeDurationSeconds = (lastWordEnd - firstWordStart) / 1000;
    }

    const wpm = activeDurationSeconds > 0 ? Math.round((wordCount / (activeDurationSeconds / 60))) : 0;
  ...
  "tip": "A short 1-sentence speaking tip (e.g. remember to pause, keep eye contact, etc)"
    ...
    try {
      const message = await anthropic.messages.create({
        model: "claude-haiku-4-5-20251001",
        max_tokens: 500,
        system: "You are a helpful debate prep assistant. Always respond with only valid JSON.",
        messages: [
          { role: "user", content: prompt }
        ]
      });
