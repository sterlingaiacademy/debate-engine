...
for (let grade = 5; grade <= 12; grade++) {
  ...
  if (!fs.existsSync(file)) {
      // try without NCERT
      file = `Grade ${grade} Social Science Benchmark Assessment (NCERT - PISA_IBT Style).txt`;
      // wait, they are all the same name format
  }
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Ignore headers
    if (line.match(/^Grade \d+/i) || line.match(/^Curriculum/i) || line.match(/^Assessment/i) || line.match(/^Section/i) || line.match(/^Explanation/i) || line.match(/^25 Multiple Choice/i) || line.match(/^This document contains/i)) {
      continue;
    }

    const correctMatch = line.match(/^(?:Correct\s+)?Answer:\s*([A-E])[.)]?\s*(.*)/i);
    if (correctMatch) {
