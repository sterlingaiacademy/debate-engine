...
conn.on('ready', () => {
  conn.exec(`echo 'wvpi2!ZnTcV];ncy' | sudo -S cp -r /home/graceandforce/debate-engine/frontend/dist/* /var/www/grace-and-force/frontend/`, (err, stream) => {
    if (err) throw err;
    stream.on('data', d => console.log('STDOUT:', d.toString()))
          .on('stderr', d => console.error('STDERR:', d.toString()))
          .on('close', () => {
             console.log('Done Copying Fixed.');
             conn.end();
          });
  });
