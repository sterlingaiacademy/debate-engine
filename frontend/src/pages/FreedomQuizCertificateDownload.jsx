...
export default function FreedomQuizCertificateDownload() {
  ...
  const handleDownload = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/freedom-quiz/certificate-status/${encodeURIComponent(email)}`);
      ...
      // "Freedom Quiz Challenge"
      const eventText = "Freedom Quiz Challenge";
      const eventSize = 16;
      ...
      saveAs(blob, `Freedom_Quiz_Certificate_${name.replace(/\s+/g, '_')}.pdf`);
      ...
      <div style={{ maxWidth: 500, width: '100%', background: '#1e293b', borderRadius: 24, padding: '2.5rem', boxShadow: '0 20px 40px rgba(0,0,0,0.4)', border: '1px solid rgba(255,255,255,0.05)', position: 'relative' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '2.5rem', marginTop: '1rem' }}>
          ...
          <p style={{ color: '#94a3b8', fontSize: '0.95rem', lineHeight: 1.6 }}>
            The Freedom Quiz Challenge has concluded. Enter your registered email address to download your certificate.
          </p>
        </div>
