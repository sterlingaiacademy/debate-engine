        ...
        <section className="rounded-[32px] p-8 md:p-12 mb-16 flex flex-col md:flex-row items-start md:items-center gap-8 bg-bg-base dark:bg-dark-base shadow-neo-portal dark:shadow-neo-dark-portal transition-all duration-300">
          ...
          <div className="flex-1">
            ...
            <div className="flex flex-wrap gap-6 md:gap-8 mt-2">
              <span className="w-max flex-shrink-0 inline-flex items-center gap-1.5 px-4 py-2 rounded-full bg-bg-base dark:bg-dark-base shadow-neo-sm-portal dark:shadow-neo-sm-dark-portal text-secondary dark:text-[#00d896] text-[13px] font-semibold transition-all duration-300 whitespace-nowrap">
                <span className="w-2 h-2 rounded-full bg-secondary dark:bg-[#00d896] animate-pulse flex-shrink-0"></span> Registered
              </span>
              {user?.classLevel && (
        ...
        <section className="mb-32">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
            {SUBJECTS.filter(s => ['English', 'Science', 'Mathematics', 'Social Sciences', 'CT & AI'].includes(s.key)).map((s) => {
              ...
              if (!isSelected) {
                return (
                  <div key={'mock-'+s.key} className="rounded-3xl p-8 bg-bg-base dark:bg-dark-base shadow-neo-inset-portal dark:shadow-neo-inset-dark-portal opacity-70 flex flex-col h-full transition-all duration-300 border-t border-white/50 dark:border-white/5">
                    ...
                    <h3 className="text-2xl font-extrabold mb-3 text-text-muted dark:text-gray-500 transition-colors duration-300">{s.label} Selection Test</h3>
              ...
              return (
                <div key={'mock-'+s.key} className="rounded-3xl p-8 bg-bg-base dark:bg-dark-base shadow-neo-inset-portal dark:shadow-neo-inset-dark-portal flex flex-col h-full border-t border-white/50 dark:border-white/5 transition-all duration-300">
                  <div className="flex justify-between items-start mb-8">
                    <div className="w-14 h-14 rounded-2xl bg-bg-base dark:bg-dark-base shadow-neo-inset-portal dark:shadow-neo-inset-dark-portal flex items-center justify-center transition-all duration-300">
                      <span className={`material-symbols-outlined ${s.iconColorClass} text-2xl opacity-60`}>{s.icon}</span>
                    </div>
                    <span className="px-4 py-1.5 rounded-full bg-bg-base dark:bg-dark-base shadow-neo-sm-portal dark:shadow-neo-sm-dark-portal text-[#ffaa00] dark:text-[#ffaa00] text-[13px] font-extrabold uppercase tracking-wider transition-all duration-300 flex items-center gap-1.5">
                      <span className="material-symbols-outlined text-[15px]">lock</span> LOCKED
                    </span>
                  </div>
                  <h3 className="text-2xl font-extrabold mb-3 text-text-muted dark:text-gray-500 transition-colors duration-300">{s.label} Selection Test</h3>
