...
export default function OlympiadEnglishQuiz({ user, subject = 'English', onClose }) {
  ...
  if (phase === 'loading') return (
    <div className="fixed inset-0 z-[100] quiz-overlay flex items-center justify-center p-4">
      <div className="text-center">
        ...
        <p className="text-white/60 font-bold">Loading {subject} Quiz...</p>
      </div>
    </div>
  );
  ...
  if (phase === 'blocked') return (
    <div className="fixed inset-0 z-[100] quiz-overlay flex items-center justify-center p-4">
      <div className="quiz-window p-10 text-center max-w-[420px] w-full shadow-neo">
        <div className="text-5xl mb-4">⚠️</div>
        <h2 className="text-white font-bold text-2xl mb-2">Cannot Load Quiz</h2>
        <p className="text-white/60 mb-8">{error}</p>
        <button onClick={onClose} style={gradientStyle} className="px-8 py-3 rounded-xl text-white font-bold shadow-neo hover:opacity-90 active:scale-95 transition-all">Close</button>
      </div>
    </div>
  );
  ...
  if (phase === 'result') {
    ...
    return (
      <div className="fixed inset-0 z-[100] quiz-overlay flex items-center justify-center p-margin-mobile md:p-margin-desktop overflow-y-auto text-white">
        <div className="quiz-window w-full max-w-[900px] relative overflow-hidden flex flex-col my-auto shadow-neo">
          {/* Top Color Bar */}
          ...
          <div className="results-main-grid grid grid-cols-[1fr_380px] min-h-[500px]">
            ...
            <div className="p-gutter border-r border-white/5 flex flex-col h-full max-h-[70vh] md:max-h-[80vh] overflow-hidden">
              <div className="mb-6 flex-shrink-0 flex items-center gap-3">
                <div className="w-10 h-10 rounded-lg bg-[#292d32] shadow-neo-sm text-white/50 flex items-center justify-center">
              ...
              <div className="flex-1 overflow-y-auto pr-2 flex flex-col gap-4">
                {breakdown.length === 0 ? (
                  <p className="text-white/40">No review data available.</p>
                ) : (
                  breakdown.map((b, i) => (
                    <div key={i} className={`p-5 rounded-2xl bg-[#292d32] ${b.isCorrect ? 'shadow-neo-inset border border-green-500/20' : 'shadow-neo border border-red-500/20'}`}>
                      <div className="flex items-start gap-4">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${b.isCorrect ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>
                          <span className="material-symbols-outlined text-[16px]">{b.isCorrect ? 'check' : 'close'}</span>
                        </div>
                        <div className="flex-1">
                          <p className="font-body-sm text-white/90 mb-2 leading-relaxed">
                            <span className="text-white/40 font-bold mr-2">Q{i+1}.</span> {b.question}
                          </p>
                          <div className="flex flex-col gap-1.5 mt-3">
                            <div className="flex items-center gap-2">
                              <span className="text-white/40 text-xs uppercase tracking-wider w-16">Selected</span>
                              <span className={`text-sm font-medium ${b.isCorrect ? 'text-green-400' : 'text-red-400'}`}>{b.selected || '—'}</span>
                            </div>
                            {!b.isCorrect && (
                              <div className="flex items-center gap-2">
                                <span className="text-white/40 text-xs uppercase tracking-wider w-16">Correct</span>
                                <span className="text-sm font-medium text-green-400">{b.correct}</span>
                              </div>
                            )}
                          </div>
                        </div>
            ...
            {/* Right Col: Summary */}
            <div className="p-gutter flex flex-col items-center justify-center bg-[#292d32] shadow-[inset_10px_0_20px_rgba(0,0,0,0.2)]">
              <div className="text-center mb-8">
                <div className="font-label-sm text-white/40 uppercase tracking-widest mb-2">ThinkQuest · {subject}</div>
                ...
                <div className="relative w-[180px] h-[180px] mx-auto mb-6 flex items-center justify-center">
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    ...
                    <div className="text-white/40 text-xs font-bold mt-1 uppercase tracking-wider">Score</div>
                  </div>
                </div>

                <div className="bg-[#292d32] shadow-neo-inset rounded-2xl p-4 inline-flex gap-8 mb-8">
                  <div className="text-center">
              ...
              <button onClick={onClose} className="w-full max-w-[280px] py-4 rounded-xl font-bold text-white shadow-neo hover:opacity-90 active:shadow-neo-btn-inset active:scale-[0.98] transition-all" style={gradientStyle}>
                Back to Dashboard
              </button>
            </div>
          </div>
  ...
  return (
    <div className="fixed inset-0 z-[100] quiz-overlay flex items-center justify-center p-margin-mobile md:p-margin-desktop overflow-y-auto text-white">
      <div className="quiz-window w-full max-w-[680px] relative overflow-hidden flex flex-col my-auto shadow-neo min-h-[500px]">
        {/* Top Color Bar */}
        <div className="h-[3px] w-full absolute top-0 left-0" style={gradientStyle}></div>
        ...
        <div className="p-gutter pb-0 pt-8 flex justify-between items-start">
          <div>
            <div className="font-label-sm text-label-sm text-white/50 tracking-wider uppercase mb-1">ThinkQuest Olympiad · {subject}</div>
            <h1 className="font-headline-md text-headline-md font-bold" style={gradientTextStyle}>{quiz.quiz_name}</h1>
          </div>
          <div className="flex gap-4">
            <div className="bg-[#292d32] shadow-neo-sm rounded-full px-4 py-1.5 flex items-center gap-1.5 hidden sm:flex">
              <span className="material-symbols-outlined text-[16px] text-white/70">format_list_numbered</span>
              <span className="font-label-sm text-label-sm text-white/90 font-bold">{answered}/{total}</span>
            </div>
            <div className={`bg-[#292d32] shadow-neo-sm rounded-full px-4 py-1.5 flex items-center gap-1.5 ${timeLeft <= 5 && !isRevealed ? 'text-red-400 danger-pulse' : 'text-blue-400'}`}>
              <span className="material-symbols-outlined text-[16px]">timer</span>
              <span className="font-label-sm text-label-sm font-bold">00:{timeLeft.toString().padStart(2, '0')}</span>
            </div>
          </div>
        </div>

        {/* Progress Bar & Navigator */}
        <div className="px-gutter py-6 border-b border-white/[0.03]">
          <div className="h-[6px] bg-[#292d32] shadow-neo-inset rounded-full w-full mb-8 overflow-hidden p-[1px]">
          ...
          <div className="flex justify-between items-center px-2 flex-wrap gap-2">
            {quiz.questions.map((_, i) => {
              ...
              if (isCur) {
                return (
                  <div key={i} className="w-[32px] h-[32px] rounded-full bg-[#292d32] shadow-neo-inset flex items-center justify-center font-bold text-sm">
              ...
              if (isOK) {
                return (
                  <div key={i} className="w-[32px] h-[32px] rounded-full bg-[#292d32] shadow-neo-sm text-green-400 flex items-center justify-center">
              ...
              if (isBAD) {
                return (
                  <div key={i} className="w-[32px] h-[32px] rounded-full bg-[#292d32] shadow-neo-sm text-red-400 flex items-center justify-center">
              ...
              if (isAns) {
                 return (
                  <div key={i} className="w-[32px] h-[32px] rounded-full bg-[#292d32] shadow-neo-sm text-white flex items-center justify-center text-xs font-medium">
                    {i + 1}
                  </div>
                );
              }
              return (
                <div key={i} className={`w-[32px] h-[32px] rounded-full bg-[#292d32] shadow-neo-sm text-white/40 items-center justify-center text-xs font-medium ${Math.abs(current - i) > 3 ? 'hidden sm:flex' : 'flex'}`}>
                  {i + 1}
                </div>
              );
            })}
        ...
        <div className="p-gutter flex-1 flex flex-col gap-8 transition-opacity duration-200" style={{ opacity: animate ? 1 : 0 }}>
          {/* Question Card */}
          <div className="bg-[#292d32] shadow-neo-inset rounded-[20px] p-6 relative overflow-hidden">
            <div className="absolute left-0 top-0 bottom-0 w-[4px] opacity-80" style={gradientStyle}></div>
            <p className="font-body-lg text-body-lg text-white/90 leading-relaxed pl-3 font-medium whitespace-pre-wrap">
              {q.question}
            </p>
          </div>
          ...
          <div className="flex flex-col gap-4 pb-4">
            {q.options.map(opt => {
              ...
              let textClass = "font-body-md text-body-md font-medium ";
              
              if (isCorrectOpt) {
                containerClass += "bg-[#292d32] shadow-neo-inset border border-green-500/20";
                letterClass += "bg-green-500/20 text-green-400 shadow-neo-inset";
                textClass += "text-green-400 font-bold";
              } else if (isWrongOpt) {
                containerClass += "bg-[#292d32] shadow-neo-inset border border-red-500/20";
                letterClass += "bg-red-500/20 text-red-400 shadow-neo-inset";
                textClass += "text-red-400 font-bold";
              } else if (isSelected) {
                containerClass += "bg-[#292d32] shadow-neo-inset";
                letterClass += "text-white shadow-[0_4px_12px_rgba(255,255,255,0.1)]"; // Custom style applied below
                textClass += "text-white font-bold";
              } else {
                containerClass += "bg-[#292d32] shadow-neo hover:shadow-neo-sm " + (isRevealed ? "opacity-50 cursor-default" : "");
                letterClass += "bg-[#292d32] shadow-neo-sm text-white/60 group-hover:text-white/80";
              ...
              return (
                <label key={opt.letter} className={containerClass}>
                  ...
                  {isCorrectOpt && <span className="material-symbols-outlined text-green-400 ml-2">check_circle</span>}
                  {isWrongOpt && <span className="material-symbols-outlined text-red-400 ml-2">cancel</span>}
                </label>
              );
            })}
          </div>
        ...
        {/* Bottom Nav */}
        <div className="p-gutter pt-4 pb-8 flex justify-between items-center bg-[#292d32] z-10 border-t border-white/5">
          ...
          {isLastQ ? (
            <button 
              ...
              className={`px-8 py-3.5 rounded-xl font-bold text-sm text-white transition-all duration-200 ${nextEnabled && !submitting ? 'shadow-neo hover:opacity-90 active:shadow-neo-btn-inset active:scale-[0.98]' : 'bg-white/5 text-white/30 cursor-not-allowed'}`}
            ...
            <button 
              ...
              className={`px-8 py-3.5 rounded-xl font-bold text-sm text-white transition-all duration-200 ${nextEnabled ? 'shadow-neo hover:opacity-90 active:shadow-neo-btn-inset active:scale-[0.98]' : 'bg-white/5 text-white/30 cursor-not-allowed'}`}
