...
// Error Boundary — catches JS crashes and shows a recovery screen instead of black
class ErrorBoundary extends React.Component {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div style={{ minHeight: '100vh', background: '#06080f', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '2rem', color: '#fff', fontFamily: 'Inter, sans-serif' }}>
          <div style={{ fontSize: '3rem', marginBottom: '1rem' }}>⚠️</div>
          <h1 style={{ fontSize: '1.5rem', fontWeight: 800, marginBottom: '0.5rem' }}>Something went wrong</h1>
          <p style={{ color: '#94a3b8', marginBottom: '1.5rem', textAlign: 'center' }}>Please refresh the page to continue.</p>
          <button onClick={() => window.location.reload()} style={{ background: '#FF6B00', color: '#fff', border: 'none', borderRadius: 10, padding: '0.75rem 1.5rem', fontWeight: 700, fontSize: '1rem', cursor: 'pointer' }}>
            Refresh Page
          </button>
          {this.state.error && <p style={{ marginTop: '1rem', fontSize: '0.75rem', color: '#475569', maxWidth: '300px', textAlign: 'center', wordBreak: 'break-word' }}>{this.state.error.message}</p>}
        </div>
      );
    }
